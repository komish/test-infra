...
Publishes the red hat universal base image based ACK service controller as part 
...
Environment variables:
  ...
  UBI_BASE_IMG:             The Red Hat Universal Base Image to use as the controller
                            image.
                            Default: $DEFAULT_RED_HAT_UBI_IMG
  UBI_BASE_IMG_TAG:         The Red Hat Universal Base Image tag to use for the controller
                            image.
                            Default: $DEFAULT_RED_HAT_UBI_IMG_TAG
  QUIET:                    Build controller container image quietly (<true|false>)
                            Default: false
"

DEFAULT_RED_HAT_UBI_IMG="ubi8/ubi-minimal"
DEFAULT_RED_HAT_UBI_IMG_TAG="8.4"
UBI_BASE_IMG=${UBI_BASE_IMG:-"$DEFAULT_RED_HAT_UBI_IMG"}
UBI_BASE_IMG_TAG=${UBI_BASE_IMG_TAG:-"$DEFAULT_RED_HAT_UBI_IMG_TAG"}
...
TARGET_RED_HAT_OSPID=$(get_red_hat_ospid $AWS_SERVICE-controller)
RED_HAT_OSPID=${TARGET_OSPID:-"$DEFAULT_OSPID"}

if [[ "$RED_HAT_OSPID" == "unknown" ]]; then
  echo "Unable to find the AWS service's corresponding OSP-ID for $AWS_SERVICE"
...
if ! (echo "$VERSION" | grep -Eq "stable$"); then
  ...
  buildah bud \
    ...
    --build-arg base_image="registry.access.redhat.com/$UBI_BASE_IMG:$UBI_BASE_IMG_TAG"
